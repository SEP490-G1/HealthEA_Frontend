  ...
  <div class="user-list">
    ...
    <div class="user-management-card card-deck mb-3"
      ...
      <div class="pricing-header text-center">
        <h2 class="display-4">{{ $t('user.list') }}</h2>
        <p><router-link to="/createUser">{{ $t('user.create') }}</router-link></p>
      </div>

      <!-- Search user -->
      <div class="search-form container">
        <form @submit.prevent="searchUsers" class="form-inline justify-content-center">
          ...
          <div class="form-group mx-2">
            <select v-model="filters.status" class="form-control">
              <option value="">{{ $t('user.status') }}</option>
        ...
        <table class="table table-bordered">
          <thead>
            <tr>
              ...
              <th scope="col">{{ $t('user.dob') }}</th>
              <th scope="col">{{ $t('user.gender') }}</th>
              <th scope="col">{{ $t('user.status') }}</th>
              <th scope="col">{{ $t('user.role') }}</th>
              <th scope="col">{{ $t('button.action') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr :key="index" v-for="(user, index) in users" :index="index">
              ...
              <td>
                <router-link :to="{ name: 'user.updateUser', params: { id: user.id } }">
                  <button class="btn btn-primary">{{ $t('button.edit') }}</button>
                </router-link>
              </td>
            </tr>
          </tbody>
      ...
      <div class="pagination-container">
        ...
        <div class="pagination">
          <button @click="changePage(currentPage - 1)" :disabled="currentPage === 0">{{ $t('page.previous') }}</button>
          <button v-for="page in totalPages" :key="page" @click="changePage(page - 1)"
            :class="{ active: currentPage === page - 1 }">
            {{ page }}
          </button>
          <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages - 1">{{ $t('page.next')
            }}</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getAllUser(page = this.currentPage, size = this.pageSize) {
      this.totalPages = Math.ceil(this.totalElements / this.pageSize)
