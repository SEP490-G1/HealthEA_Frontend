<template>
    <div class="verification-container">
        <div class="verification-card">
            ...
            <p v-if="status === 'success'">Your email has been verified successfully!</p>
            <p v-if="status === 'error'">Invalid or expired token.</p>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        verifyToken(token) {
            this.$request.post(`/emails/verifyEmail/` + token)
                .then(() => {
                    this.$swal({
                        title: 'Success!',
